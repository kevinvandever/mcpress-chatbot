#!/usr/bin/env python3
"""
Local PDF Preprocessing Script for MC Press Books
Processes all PDFs locally and uploads vectors to Railway PostgreSQL
"""
...
class LocalPreprocessor:
    ...
    async def store_to_database(self, book_data: Dict[str, Any], conn: asyncpg.Connection):
        """Store processed book data to PostgreSQL"""
        try:
            # Insert book record
            ...
            embeddings_data = [
                (book_id, i, emb['text'], emb['embedding'], emb['page'])
    ...
    async def run(self):
        ...
        print("🚀 MC Press Books Local Preprocessor")
        print(f"📁 Processing PDFs from: {self.uploads_dir}")
        print(f"🗄️  Storing to: Railway PostgreSQL")
